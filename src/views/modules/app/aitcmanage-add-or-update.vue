    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/app/aitcmanage/info/${this.dataForm.id}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 1) {
                this.dataForm.name = data.aitcmanage.name
                this.dataForm.type = data.aitcmanage.type
                this.dataForm.rpcuser = data.aitcmanage.rpcuser
                this.dataForm.rpcpassword = data.aitcmanage.rpcpassword
                this.dataForm.ip = data.aitcmanage.ip
                this.dataForm.port = data.aitcmanage.port
                this.dataForm.status = data.aitcmanage.status
                this.dataForm.address = data.aitcmanage.address
                this.dataForm.createTime = data.aitcmanage.createTime
                this.dataForm.updateTime = data.aitcmanage.updateTime
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/app/aitcmanage/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',
